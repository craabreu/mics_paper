...
filebase = 'composition_sensitivity_test'
...
T = 300.0 # K
df_mics = kB*T*data['df_mics']
df_mics_ref = kB*T*0.13286765

n, bins, patches = plt.hist(df_mics, 30, histtype='bar', log=False, cumulative=False,
                            density=True, edgecolor='black', facecolor='green', alpha=0.25)

mean, stdev = stats.norm.fit(df_mics)

x = np.linspace( df_mics.min(), df_mics.max(), num=200)

plt.plot(x, stats.norm.pdf(x, mean, stdev), linewidth=1.0,
         color='green', label='log-normal fit')

plt.plot([df_mics_ref], [0.0], 'o', markersize=2, color='black', clip_on=False)
ax.annotate(r'$\pi_i \propto n_i^{eff}$', xy=(df_mics_ref, 0.0), xytext=(0.079125, 6000), ha='center',
